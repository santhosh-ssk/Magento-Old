       ...
       \Magento\Customer\Model\Session $customerSession)
{
    ...
    public function execute()
    {
      ...
      if($this->_customerSession->isLoggedIn()){
         ...
         if($productId){
            try{
                $item = $_customerFavoriteItems->getCollection()->addFieldToFilter('customer_id',array('eq' => $customerId))->addFieldToFilter('product_id',array('eq' => $productId));
                $entity_id = $item->toArray()['items'][0]['entity_id'];//get_class_methods();
                $_customerFavoriteItems->load($entity_id);
                $_customerFavoriteItems->delete();

                $responseObj['message'] = 'success';
                $responseObj['isSuccess'] = true;
            }
            catch( \Exception $e){
               $responseObj['message'] = 'error in deleting data';   
            }
         }
         else{
